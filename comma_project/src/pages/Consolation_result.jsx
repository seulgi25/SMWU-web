 ...
 * 추천 글귀 DB
 *
 * 사용자가 선택한 감정(emotion) 또는 상황(situation)이
 * 각 글귀의 tags 배열에 포함되어 있으면 추천 후보가 됩니다.
 *
 * 예)
 * 선택 감정: '불안'
 * 선택 상황: '진로 고민'
 * → tags에 '불안' 또는 '진로 고민'이 포함된 글귀 중 하나를 랜덤 추천
 */
const QUOTES_DB = [
  // 감정: 지침 / 번아웃, 무기력
  {
...
/**
 * 진단 태그와 대나무숲 태그를 연결하는 매핑입니다.
 * 대나무숲 글의 태그는 #학업, #취업, #인간관계처럼 단순하게 유지하고,
 * 진단 결과의 세부 상황만 내부적으로 대표 태그와 연결합니다.
 */
...
const addTagVariants = (tagSet, value) => {
  const trimmedValue = String(value ?? '').trim();

  if (!trimmedValue) return;

  const normalizedValue = normalizeTagKey(trimmedValue);
  const hashTag = toHashTag(trimmedValue);
  const normalizedHashTag = toHashTag(normalizedValue);
  const mappedTag = getCommunityTag(trimmedValue);
  const mappedPlainTag = mappedTag?.replace(/^#/, '');

  [trimmedValue, normalizedValue, hashTag, normalizedHashTag, mappedTag, mappedPlainTag].forEach(
    (tag) => {
...
const buildCommunitySearchTags = ({ emotion, situation }) => {
  const searchTags = new Set();

  addTagVariants(searchTags, emotion);
  addTagVariants(searchTags, situation);

  return Array.from(searchTags).slice(0, 10);
};
...
const fetchCurrentWeather = async (lat, lng) => {
  ...
  try {
    ...
    return response.data?.weather?.[0]?.main || 'Clear';
  } catch (error) {
...
const fetchMatchedPosts = async (userState) => {
  ...
  const searchTags = buildCommunitySearchTags(userState);

  const tagQuery = query(postsRef, where('tag', 'in', searchTags));
  const tagsQuery = query(postsRef, where('tags', 'array-contains-any', searchTags));

  const postMap = new Map();
  const queryResults = await Promise.allSettled([getDocs(tagQuery), getDocs(tagsQuery)]);

  queryResults.forEach((result) => {
    if (result.status !== 'fulfilled') {
      console.error('대나무숲 추천 쿼리 실패:', result.reason);
      return;
    }

    result.value.docs.forEach((postDoc) => {
      postMap.set(postDoc.id, { id: postDoc.id, ...postDoc.data() });
    });
  });

  return sortPostsByLatest(Array.from(postMap.values()));
};
...
const Consolation_result = () => {
  ...
  // 대나무숲 게시글과 댓글에 첨부된 음악을 분석하여 맞춤 추천을 만든다.
  useEffect(() => {
    let isActive = true;
  ...
  // 현재 위치와 날씨를 기반으로 카카오맵에서 방문 가능한 힐링 공간을 검색한다.
  useEffect(() => {
