 ...
 *
 * 익명 대나무숲 게시글 목록 페이지입니다.
 * Firestore의 secret_forest_list 컬렉션에서 게시글을 최신순으로 불러오고,
 * 한 페이지당 15개씩 나누어 보여줍니다.
 *
 * - 게시글 클릭: 해당 게시글 상세 페이지로 이동
 * - 글 작성하기: 게시글 작성 페이지로 이동
 * - 게시글이 없을 경우: 빈 목록 안내 문구 표시
 */
...
const formatTag = (post) => {
  ...
  // 기존 DB에 저장된 '#학업스트레스', '#학업 스트레스'를 화면에서는 '#학업'으로 통일한다.
...
const SecretForestList = () => {
  ...
  // Firestore 게시글 목록을 최신순으로 실시간 구독한다.
  useEffect(() => {
  ...
  // 게시글 수가 줄어 현재 페이지가 사라지는 경우 마지막 페이지로 이동한다.
  useEffect(() => {
